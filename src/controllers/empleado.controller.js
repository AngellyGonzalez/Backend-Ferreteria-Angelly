...
export const obtenerEmpleado = async (req, res) => {
    try {
        ...
        const [result] = await pool.query('SELECT * FROM empleados WHERE id_empleado = ?', [req.params.id_empleado]);
        if (result.length <= 0) {
            return res.status(404).json({
                mensaje: `Error al leer los datos. ID ${req.params.id_empleado} no encontrado.`
            });
        }
        res.json(result[0]);
    } catch (error) {
...
export const registrarEmpleado = async (req, res) => {
    ...
    fecha_contratacion
        ...
        const [result] = await pool.query(
            'INSERT INTO empleado (  primer_nombre, segundo_nombre, primer_apellido, segundo_apellido,celular, cargo, fecha_contratacion) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [  primer_nombre,
    segundo_nombre,
    primer_apellido,
    segundo_apellido,
...
// Actualizar una Empleado por id 
